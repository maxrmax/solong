...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/02/07 14:30:16 by mring             #+#    #+#             */
/*   Updated: 2024/02/07 16:26:32 by mring            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	print_hex_ptr(unsigned long number, int hex, int ptr)
{
	...
	{
		count += print_hex_ptr(number / 16, hex, 0);
		if (count == -1)
			return (-1);
		return (count += print_hex_ptr(number % 16, hex, 0));
	}
}
